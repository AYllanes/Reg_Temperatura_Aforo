...
void POS_CURSOR(unsigned char fila,unsigned char col)
{
    if(fila == 1){
        ENVIA_LCD_CMD(0x7F+col);
        gps = gps + col - 1;
    }
    else if(fila == 2){
        ENVIA_LCD_CMD(0x80+0x3F+col);
        gps = gps + 0x3F + col;
    }
    else if(fila == 3){
        ENVIA_LCD_CMD(0x80+0x13+col);
        gps = gps + 0x13 +col;
    }
    else if(fila == 4){
        ENVIA_LCD_CMD(0x80+0x53+col);
        gps = gps + 0x53 +col;
    }
}
...
void GENERACARACTER(const unsigned char *vector,unsigned char pos)
{
	...
	ENVIA_LCD_CMD(0x80+gps);	//Dirección de la DDRAM
}
